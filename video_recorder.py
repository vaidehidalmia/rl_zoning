...
def create_video_of_agent(
    model_path, save_path="agent_performance", num_episodes=3, format="gif"
):
    """
    ...
    for episode in range(num_episodes):
        ...
        for step in range(100):  # Max 100 steps
            ...
            if terminated or truncated:
                print(
                    f"Episode {episode + 1}: {'✅ COMPLETED' if terminated else '❌ TIMEOUT'} in {step + 1} steps (reward: {total_reward:.1f})"
                )
                break
        ...
        for step in range(100):
            ...
            if terminated or truncated:
                if terminated and step + 1 <= 20:  # Good performance
                    ...
                    print(
                        f"🏆 Recorded excellent episode: {filename} ({step + 1} steps)"
                    )
                    return
                else:
                    print(
                        f"Episode {episode}: {step + 1} steps (looking for better...)"
                    )
                break
